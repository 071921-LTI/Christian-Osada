...
let apiURL = 'https://goweather.herokuapp.com/weather/Enfield';
...
function populateData(response) {
   ...
   let todayTag = document.createElement('h2');
   todayTag.innerHTML = 'Today: ';
   let tempTag = document.createElement('h3');
   tempTag.innerHTML = response.temperature;
   let windTag = document.createElement('h3');
   windTag.innerHTML = response.wind;
   let descTag = document.createElement('h3');
   descTag.innerHTML = response.description;
   
   let forecastArray = response.forecast;
   let forecasts = document.createElement('ul');

   // Appending list elements to a ul
   for(let forecast of forecastArray){
       let forecastLi = document.createElement('li');
       forecastLi.innerHTML = forecast.temperature;
       forecasts.appendChild(forecastLi);
   }

   // Appending h3 and List to the section tag
   dataSection.appendChild(todayTag);
   dataSection.appendChild(tempTag);
   dataSection.appendChild(windTag);
   dataSection.appendChild(descTag);
   dataSection.appendChild(forecasts);
